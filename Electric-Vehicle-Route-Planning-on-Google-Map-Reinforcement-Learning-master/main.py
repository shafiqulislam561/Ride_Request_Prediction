...
print("------------Parameter --------------")
path = "C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\model"
pre_train = pre_train_step  # don't update and train the model within these steps
train_num = 3   # total episode num
...
############### Load Model
pathload = "C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\Result\\47_proceed46\\model"
load_model = False
modelnum = 27
sleep = False
if load_model == True:
...
if load_model == True:
    with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffercurrents1.txt", "r") as f:
        buffercurrents1 = f.readlines()
        buffercurrents1 = [x.strip() for x in buffercurrents1]
    with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffercurrents2.txt", "r") as f:
        buffercurrents2 = f.readlines()
        buffercurrents2 = [x.strip() for x in buffercurrents2]
    with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\bufferactions.txt", "r") as f:
        bufferactions = f.readlines()
        bufferactions = [x.strip() for x in bufferactions]
    with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffernexts1.txt", "r") as f:
        buffernexts1 = f.readlines()
        buffernexts1 = [x.strip() for x in buffernexts1]
    with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffernexts2.txt", "r") as f:
        buffernexts2 = f.readlines()
        buffernexts2 = [x.strip() for x in buffernexts2]
    with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\bufferrewards.txt", "r") as f:
        bufferrewards = f.readlines()
        bufferrewards = [x.strip() for x in bufferrewards]
    with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\bufferds.txt", "r") as f:
...
df_1.to_csv("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\train_para.csv", header=["google map_boundary", "map_height(km)", "map_wide(km)", "step_length(m)", "wide_grid_num", "height_grid_num", "total_point", "max_train_step in episode", "pre_train_step", "learning_rate", "Google r", "Google charge num", "Google SOC", "Google time"])
...
with tf.Session() as sess:
    ...
    for episode in range(train_num):  # num of episode
	    ...
	    # how to choose action 
            ...
            if d == True:
                ...
                if episode == 0:                    
                    ...
                    df.to_csv("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\result.csv", header=["episode", "step", "time", "reward", "reward_notrain", "reward_nofail", "charge_num", "SOC", "unreach_position", "Reach", "Random_a", "Avg_loss", "overQuery_num", "Loss history", "Step history"])
                    tt = tt + 1
                elif episode > 0:
                    with open('C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\result.csv', 'a') as f:
                        df = pd.DataFrame([history])
        ...
        with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffercurrents1.txt", "w") as ff:    # save the replay buffer
            for s in replay_buffer.currents1:
                ff.write(str(s) + "\n")
        with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffercurrents2.txt", "w") as mm:    # save the replay buffer
            for s in replay_buffer.currents2:
                mm.write(str(s) + "\n")
        with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\bufferactions.txt", "w") as gg:    # save the replay buffer
            for s in replay_buffer.actions:
                gg.write(str(s) + "\n")
        with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\bufferrewards.txt", "w") as hh:    # save the replay buffer
            for s in replay_buffer.rewards:
                hh.write(str(s) + "\n")
        with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffernexts1.txt", "w") as ii:    # save the replay buffer
            for s in replay_buffer.nexts1:
                ii.write(str(s) + "\n")
        with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\buffernexts2.txt", "w") as kk:    # save the replay buffer
            for s in replay_buffer.nexts2:
                kk.write(str(s) + "\n")
        with open("C:\\Users\\HP\Desktop\\django_neuralnets\\Electric-Vehicle-Route-Planning-on-Google-Map-Reinforcement-Learning-master\\bufferds.txt", "w") as jj:    # save the replay buffer
